...
Matrix3x3 parse_transformation(const json &node) {
    ...
    // For each transform entry in the array, build its 3x3 matrix M
    // and compose it with F as F = M * F so the order in JSON is preserved.
    for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
        if (auto scale_it = it->find("scale"); scale_it != it->end()) {
            ...
            M(0,0) = scale.x; M(1,1) = scale.y; M(2,2) = Real(1);
            F = M * F;
        } else if (auto rotate_it = it->find("rotate"); rotate_it != it->end()) {
            // angle is in degrees in the JSON; convert to radians
            Real angle_deg = *rotate_it;
            Real angle = angle_deg * (M_PI / Real(180.0));
            ...
            Matrix3x3 M = Matrix3x3::identity();
            // shear x: x' = x + shear_x * y  => [1 shear_x 0; 0 1 0; 0 0 1]
            M(0,1) = shear_x;
            F = M * F;
        } else if (auto sheary_it = it->find("shear_y"); sheary_it != it->end()) {
            Real shear_y = *sheary_it;
            Matrix3x3 M = Matrix3x3::identity();
            // shear y: y' = y + shear_y * x  => [1 0 0; shear_y 1 0; 0 0 1]
            M(1,0) = shear_y;
            F = M * F;
        }  
    }
