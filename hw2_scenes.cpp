...
Matrix4x4 parse_transformation(const json &node) {
    ...
    for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
        if (auto scale_it = it->find("scale"); scale_it != it->end()) {
            ...
            R(0, 0) = axis.x * axis.x + (1 - axis.x * axis.x) * c;
            R(0, 1) = axis.x * axis.y * (1 - c) - axis.z * s;
            R(0, 2) = axis.x * axis.z * (1 - c) + axis.y * s;
            
            R(1, 0) = axis.y * axis.x * (1 - c) + axis.z * s;
            R(1, 1) = axis.y * axis.y + (1 - axis.y * axis.y) * c;
            R(1, 2) = axis.y * axis.z * (1 - c) - axis.x * s;
            
            R(2, 0) = axis.z * axis.x * (1 - c) - axis.y * s;
            R(2, 1) = axis.z * axis.y * (1 - c) + axis.x * s;
            R(2, 2) = axis.z * axis.z + (1 - axis.z * axis.z) * c;
